...
8. **Dev/creator** — dev holding %, dev transfers, creator launch scan (classify by liquidity) → `analyze_dev`. M18: a fresh persisted deployer reputation (within `deployer_reputation_ttl_hours`) is reused, skipping the live per-launch scan; a freshly-scanned deployer with real history is persisted to the `deployers` table.
9. **Wallet intel** — build `known_contracts` (LP + contract holders) → `analyze_buy_timing` (M15: same-block / launch-window buy cohort from the already-fetched transfers) → `profile_token_wallets` (insiders; M16: smart candidates get a bounded cross-token survival lookup so the proxy can clear its threshold; persists) → `_watchlist_hits` (M17: each hit enriched with `prior_tokens` from the persisted `wallet_activity` memory, feeding the reputation signal in `scoring`).
